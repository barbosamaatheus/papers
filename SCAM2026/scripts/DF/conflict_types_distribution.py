...
from df_reporter import DFConflictReporter

@staticmethod
def _classify_1_1(left_files, right_files):
    return "DF_A1: left A, right A, conflict inside A" if left_files[0] == right_files[0] else "DF_A2: left A, right B, conflict between A and B"

@staticmethod
def _classify_1_2(left_files, right_files):
    if left_files[0] == right_files[0] and left_files[0] != right_files[-1]:
        return "DF_B2: one side A, other A-B, conflict between A and B"
    if left_files[0] == right_files[-1]:
        return "DF_C2: one side B, other A-B, conflict inside B"
    if left_files[0] != right_files[0] and left_files[0] != right_files[-1]:
        return "DF_A3: one side A, other B-C, conflict between A and C"
    return "Other cases"
...
def _classify_2_2(left_files, right_files):
    ...
    if left1 == right1:
        if left0 == right0:
            return "DF_D2: left A-B, right A-B, conflict inside B"
        return "DF_B3: left A-C, right B-C, conflict inside C"

    # shared first file
    if left0 == right0:
        return "DF_C3: left A-B, right A-C, conflict between B and C"

    # reversed sources: left A-B, right B-A (must precede chain check)
    if left0 == right1 and left1 == right0:
        return "DF_E2: left A-B, right B-A, conflict between A and B"

    # chained flows: one side's last file equals the other side's first
    if left1 == right0 or left0 == right1:
        return "DF_D3: left A-B, right B-C, conflict between B and C"

    return "DF_A4: left A-B, right C-D, conflict between B and D"
...
def generate_scenario_report_df(df_conflicts, modified_lines, scenario_idx=0):
    """Process a single scenario's DF conflicts and return aggregated results.
    ...
    for conflict in df_conflicts:
        try:
            print(f"Processing DF: scenario {scenario_idx}, conflict {conflict_idx}")
            ...
            processed = conflict_analyzer._process_interference_df(interference, effective_modified_lines)
            ...
            # Find source and sink nodes for DF conflict
            left_node = None
            right_node = None
            ...
            metrics = conflict_analyzer.calculate_conflict_metrics(left_node, right_node)
...
def generate_report_df(types_counter, details, output_dir=None):
    """Process all DF conflicts from all scenarios and generate combined report."""
    ...
    if output_dir is None or not os.path.exists(output_dir):
        ...
        df_dir = os.path.join(output_dir, "DF")
        os.makedirs(df_dir, exist_ok=True)
        output_pdf = os.path.join(df_dir, "conflict_types_report.pdf")
    ...
    reporter = DFConflictReporter(df_dir)
